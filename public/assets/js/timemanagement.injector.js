...
(function () {
  ...
  function ensureStyles() {
    ...
    s.textContent = `
      ...
      .tm-item-sub { font-family: var(--font-mono); font-size: 0.65rem; color: var(--text-dim); padding-left: 0; }
      .tm-item-actions { display: flex; gap: 0.25rem; margin-top: 0.1rem; align-self: flex-start; }
      .tm-action-btn { background: none; border: 1px solid var(--border); border-radius: 4px; font-size: 0.68rem; width: 22px; height: 22px; display: flex; align-items: center; justify-content: center; cursor: pointer; transition: all 0.15s; color: var(--text-dim); flex-shrink: 0; padding: 0; }
  ...
  function renderKpi() {
    ...
    const todayItems    = items.filter(i => i.pane === 'Schedule' && i.date === todayStr);
    const schedHit      = todayItems.filter(i => i.done).length;
    ...
    const projectMap = {};
    items.filter(i => i.pane === 'Do' && i.source === 'project' && i.date === todayStr && !i.done)
      .forEach(i => {
        const key = i.project_id || 'unknown';
        if (!projectMap[key]) projectMap[key] = { name: i.project_name || key, count: 0 };
        projectMap[key].count++;
      });
    const projChips = Object.values(projectMap)
      .map(p => `<span class="tm-chip-val" style="font-size:0.75rem">${truncate(p.name, 12)} · ${p.count}</span>`)
      .join(' ');
    ...
    return `
      <div class="tm-kpi-strip tm-zone">
        ...
        <div class="tm-chip" style="min-width:160px">
          <span class="tm-chip-label">PROJECT TASKS TODAY</span>
          <div style="display:flex;flex-wrap:wrap;gap:0.3rem;margin-top:0.1rem">${projChips || '<span class="tm-chip-val">—</span>'}</div>
        </div>
      </div>`;
  }
  ...
  function renderFlow() {
    ...
    function nodeHtml(item) {
      ...
      const time = item.schedule_time ? ' ' + item.schedule_time : '';
      const tip  = `${item.title} · ${item.pane || ''}${item.date ? ' · ' + item.date : ''}`;
      return `<span class="${cls}" title="${tip}">${lbl}${time}</span>`;
    }
    ...
    if (flowView === 'today') {
      ...
      nodesHtml = `<div class="tm-flow-nodes">${
        flowItems.length ? flowItems.map(nodeHtml).join('') : '<span style="font-family:var(--font-mono);font-size:0.72rem;color:var(--text-dim)">No flow items today.</span>'
      }</div>`;
    } else {
        ...
        <div class="tm-flow-header">
          ...
          <div style="display:flex;gap:0.4rem;align-items:center">
            <div class="tm-flow-toggle">
              <button class="tm-flow-btn ${flowView === 'today' ? 'active' : ''}" data-fv="today">Today</button>
              <button class="tm-flow-btn ${flowView === 'week'  ? 'active' : ''}" data-fv="week">This Week</button>
            </div>
            <button id="tm-routine-btn" style="font-family:var(--font-mono);font-size:0.68rem;padding:0.2rem 0.55rem;background:rgba(59,130,246,0.08);border:1px solid rgba(59,130,246,0.3);border-radius:var(--radius);color:#3b82f6;cursor:pointer">⚙ Routine</button>
          </div>
        </div>
        ${nodesHtml}
      </div>`;
  ...
  function renderDoIt() {
    ...
    function doitCard(item) {
      ...
      return `<div class="tm-doit-card ${isProject ? 'tm-project-card' : ''}" style="border-radius:4px;padding:0.3rem 0.4rem">
        ...
        <span class="${isProject ? 'tm-project-title' : ''}" style="font-size:0.8rem;flex:1" title="${item.title}">${truncate(item.title, 24)}</span>
        <button class="tm-action-btn done-btn" data-id="${item.id}" data-action="done" title="Mark done">✓</button>
        <button class="tm-action-btn del-btn"  data-id="${item.id}" data-action="del"  title="Remove">✕</button>
      </div>`;
    }

    const doItems = items
      .filter(i => i.pane === 'Do' && (i.date === todayStr || (i.high_impact && !i.done)))
      .filter(i => !i.done)
      .sort((a, b) => (b.high_impact ? 1 : 0) - (a.high_impact ? 1 : 0))
      .slice(0, 5);

    const buyItems = items
      .filter(i => i.pane === 'BuyPay' && (i.date === todayStr || (i.high_impact && !i.done)))
      .filter(i => !i.done)
      .sort((a, b) => (b.high_impact ? 1 : 0) - (a.high_impact ? 1 : 0))
      .slice(0, 5);
  ...
  function renderCalendar() {
    ...
    const clearBtn = calSelected
      ? `<button id="tm-cal-clear" style="font-family:var(--font-mono);font-size:0.68rem;padding:0.15rem 0.45rem;background:rgba(245,197,24,0.08);border:1px solid rgba(245,197,24,0.3);border-radius:var(--radius);color:var(--yellow);cursor:pointer">✕ Clear filter</button>`
      : '';
      ...
      <div class="tm-cal tm-zone">
        <div class="tm-cal-header">
          ...
          <div style="display:flex;gap:0.35rem;align-items:center">
            ${clearBtn}
            <div class="tm-flow-toggle">
              <button class="tm-flow-btn ${calView === 'month' ? 'active' : ''}" data-cv="month">Month</button>
              <button class="tm-flow-btn ${calView === 'week'  ? 'active' : ''}" data-cv="week">Week</button>
            </div>
          </div>
        </div>
        ${gridHtml}
  ...
  function itemCard(item) {
    ...
    if (item.schedule_time) sub += (sub ? ' · ' : '') + item.schedule_time;
    ...
    const readOnly = isProject;
    const editBtn  = !readOnly ? `<button class="tm-action-btn" data-id="${item.id}" data-action="edit" title="Edit" style="font-size:0.6rem">✎</button>` : '';

    return `<div class="tm-item-card ${isProject ? 'tm-project-card' : ''}" data-id="${item.id}">
      <div class="tm-item-top">
        <span>${forceMark}${impactMark}${doneMark}</span>
        <span class="tm-item-title ${item.done ? 'done-title' : ''} ${isProject ? 'tm-project-title' : ''}" title="${item.title}">${truncate(item.title, 28)}</span>
  ...
  function addForm(pane) {
    ...
    const budgetOptions = budgets
      .filter(b => b.fields ? b.fields.category_type === 'Expense' : (b.category_type === 'Expense'))
      .map(b => {
        const lbl = b.fields ? (b.fields.label || b.fields.name || b.id) : (b.label || b.id);
        const id  = b.id;
    ...
    return `<div class="tm-inline-form" data-form-pane="${pane}">
      ...
      ${isSchedule ? `
        <div class="tm-form-row">
          <input type="text" id="tm-form-time"  placeholder="HH:MM" inputmode="numeric">
      ...
      ${isBuyPay ? `
        <div class="tm-form-row">
          ...
          <select id="tm-form-budget"><option value="">— Budget —</option>${budgetOptions}</select>
        </div>
        <input type="date" id="tm-form-date" style="margin-bottom:0.35rem">
      ` : ''}
      ...
      <div class="tm-form-btns">
        <button class="tm-save-btn" data-save-pane="${pane}">Save</button>
        <button class="tm-cancel-btn" data-cancel-pane="${pane}">Cancel</button>
      </div>
    </div>`;
  }
  ...
  function renderPanes() {
    ...
    return `<div class="tm-panes-area">${panes.map(pane => {
      ...
      // Float overdue + high_impact to top, then sort newest
      paneItems.sort((a, b) => {
      ...
      const body = paneItems.length
        ? paneItems.map(itemCard).join('')
        : `<div class="tm-empty">Nothing here. Tap + to add.</div>`;
      ...
      return `<div class="tm-pane" data-pane="${pane}">
        <div class="tm-pane-header">
          ...
          <button class="tm-add-btn" data-add-pane="${pane}">+ Add</button>
        </div>
  ...
  function wireEvents(root) {
    ...
    root.addEventListener('click', async function (e) {
      ...
      if (cancelPane) { removeForm(cancelPane); return; }
      ...
      // DO IT strip buttons (outside panes area)
      const doItAction = t.dataset.action;
      const doItId     = t.dataset.id;
      if (doItAction && doItId && t.closest('.tm-doit-strip')) {
        if (doItAction === 'done') await markDone(doItId, t);
        if (doItAction === 'del')  await removeItem(doItId, t);
      }
    });
  }

  function openAddForm(pane) {
    const p         = panel();
  ...
  function openEditForm(id) {
    ...
    // Re-use addForm structure but pre-fill with existing values
    ...
    // Replace save button behavior for edit
    ...
    form.addEventListener('click', async function (ev) {
      ...
      if (ev.target.dataset.cancelPane) { await loadAndRender(); }
    });
  }
  ...
  function showRoutineModal() {
    ...
    const listHtml = routines.length
      ? routines.map(r => `
          <div style="display:flex;align-items:center;justify-content:space-between;padding:0.4rem 0;border-bottom:1px solid var(--border)">
            ...
            <button style="background:none;border:1px solid var(--red);border-radius:4px;color:var(--red);font-size:0.68rem;padding:0.15rem 0.45rem;cursor:pointer" data-del-routine="${r.id}">✕</button>
          </div>`).join('')
    ...
    modal.innerHTML = `<div class="tm-routine-card">
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:0.75rem">
        ...
        <button id="tm-routine-close" style="background:none;border:1px solid var(--border);border-radius:4px;padding:0.2rem 0.5rem;cursor:pointer;color:var(--text-muted)">✕</button>
      </div>
      ...
      <div style="margin-top:1rem;border-top:1px solid var(--border);padding-top:0.75rem">
        ...
        <input type="text" id="tm-routine-time" placeholder="HH:MM (optional)" inputmode="numeric" style="margin-bottom:0.4rem">
        <button id="tm-routine-save" style="font-size:0.78rem;padding:0.3rem 0.75rem;background:var(--yellow);color:#0a0a10;border:none;border-radius:var(--radius);cursor:pointer;font-weight:600">Add</button>
      </div>
    </div>`;
